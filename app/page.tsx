...
const PROBLEMS = [
  { key: 'outreach', label: '不知道怎么找客户', icon: Send, tool: '拓客信 ProspectKit', note: '从目标画像和公开触发信号出发，生成今天、+2、+5、+9 天四步冷启动序列；不是群发。', href: 'https://chenxiaolong-maxgent.github.io/prospectkit-cn/', action: '免费生成拓客序列', paid: '定制拓客冲刺', price: '¥199 起', paidHref: 'https://github.com/chenxiaolong-maxgent/prospectkit-cn/issues/new?template=outreach-sprint-service.yml' },
  { key: 'sample', label: '不会先给小样', icon: FileText, tool: '诊断页 AuditPage', note: '只基于公开、相关、可核对的材料，把一条观察、一个待验证判断和一个低成本行动压成一页。', href: 'https://chenxiaolong-maxgent.github.io/auditpage-cn/', action: '免费生成一页诊断', paid: '定制一页诊断', price: '¥199 起', paidHref: 'https://github.com/chenxiaolong-maxgent/auditpage-cn/issues/new?template=custom-audit-service.yml' },
  { key: 'pilot', label: '客户不敢签大单', icon: ClipboardCheck, tool: '试单匠 PilotOffer', note: '把模糊的大项目压缩成一个范围清楚、价格明确、能支持下一步决策的付费试单。', href: 'https://chenxiaolong-maxgent.github.io/pilotoffer-cn/', action: '免费生成试单方案', paid: '定制试单设计', price: '¥199 起', paidHref: 'https://github.com/chenxiaolong-maxgent/pilotoffer-cn/issues/new?template=custom-pilot-service.yml' },
  { key: 'brief', label: '报价前没问清', icon: ScanSearch, tool: '问需 BriefPilot', note: '把模糊期待整理成访谈议程、优先问题、风险提醒、会议纪要和可确认的需求基线。', href: 'https://chenxiaolong-maxgent.github.io/briefpilot-cn/', action: '免费生成访谈包', paid: '访谈方案定制', price: '¥99 起', paidHref: 'https://github.com/chenxiaolong-maxgent/briefpilot-cn/issues/new?template=discovery-service.yml' },
  { key: 'capacity', label: '项目一多就超载', icon: CalendarRange, tool: '排期眼 CapacityFlow', note: '把已有排期和新机会放进同一张四周负载地图，看清超载周、剩余产能和安全开工窗口。', href: 'https://chenxiaolong-maxgent.github.io/capacityflow-cn/', action: '免费计算四周产能', paid: '项目产能系统配置', price: '¥99 起', paidHref: 'https://github.com/chenxiaolong-maxgent/capacityflow-cn/issues/new?template=capacity-setup-service.yml' },
  { key: 'pipeline', label: '线索多但没预测', icon: Target, tool: '成单板 DealBoard', note: '把五个阶段的商机金额按成交率加权，看清本月预测、目标覆盖率和收入缺口。', href: 'https://chenxiaolong-maxgent.github.io/dealboard-cn/', action: '免费计算成交预测', paid: '销售管道配置', price: '¥99 起', paidHref: 'https://github.com/chenxiaolong-maxgent/dealboard-cn/issues/new?template=pipeline-setup-service.yml' },
  { key: 'objection', label: '客户一说贵就降价', icon: MessagesSquare, tool: '异议卡 ObjectionDeck', note: '先判断客户担心预算、价值、时机、比较还是决策风险，再生成确认、诊断、重构和下一步四段回应。', href: 'https://chenxiaolong-maxgent.github.io/objectiondeck-cn/', action: '免费生成异议回复', paid: '客户异议方案', price: '¥99 起', paidHref: 'https://github.com/chenxiaolong-maxgent/objectiondeck-cn/issues/new?template=objection-plan-service.yml' },
  ...
  { key: 'feedback', label: '客户反馈太散', icon: ListChecks, tool: '评改 ReviewDock', note: '把微信、邮件和批注里的多人意见收口成反馈规则、冲突决定、改稿清单与版本确认。', href: 'https://chenxiaolong-maxgent.github.io/reviewdock-cn/', action: '免费收口反馈', paid: '客户反馈工作流定制', price: '¥99 起', paidHref: 'https://github.com/chenxiaolong-maxgent/reviewdock-cn/issues/new?template=feedback-workflow-service.yml' },
  { key: 'status', label: '客户总来追进度', icon: RadioTower, tool: '进度通 StatusPulse', note: '把已完成、下一步、风险、客户配合和里程碑整理成可直接发送的周报或邮件。', href: 'https://chenxiaolong-maxgent.github.io/statuspulse-cn/', action: '免费生成进度汇报', paid: '客户汇报工作流定制', price: '¥99 起', paidHref: 'https://github.com/chenxiaolong-maxgent/statuspulse-cn/issues/new?template=client-reporting-service.yml' },
...
const CATALOG: CatalogItem[] = [
  { name: '拓客信 ProspectKit', type: 'free' as const, price: '免费', description: '按目标画像、公开触发信号与相关证据生成四步冷启动拓客序列。', href: 'https://chenxiaolong-maxgent.github.io/prospectkit-cn/', icon: Send },
  { name: '诊断页 AuditPage', type: 'free' as const, price: '免费', description: '把公开观察、待验证判断、低成本行动和验证信号整理成一页诊断。', href: 'https://chenxiaolong-maxgent.github.io/auditpage-cn/', icon: FileText },
  { name: '试单匠 PilotOffer', type: 'free' as const, price: '免费', description: '生成范围、交付、价格、反馈边界、验证信号与下一步都清楚的一页付费试单。', href: 'https://chenxiaolong-maxgent.github.io/pilotoffer-cn/', icon: ClipboardCheck },
  { name: '问需 BriefPilot', type: 'free' as const, price: '免费', description: '生成需求访谈议程、问题、会前消息、风险提醒、会议纪要与需求确认。', href: 'https://chenxiaolong-maxgent.github.io/briefpilot-cn/', icon: ScanSearch },
  { name: '排期眼 CapacityFlow', type: 'free' as const, price: '免费', description: '计算未来四周负载、超载工时、剩余产能与新项目安全窗口。', href: 'https://chenxiaolong-maxgent.github.io/capacityflow-cn/', icon: CalendarRange },
  { name: '成单板 DealBoard', type: 'free' as const, price: '免费', description: '按商机阶段概率计算加权成交预测、目标覆盖率与收入缺口。', href: 'https://chenxiaolong-maxgent.github.io/dealboard-cn/', icon: Target },
  { name: '异议卡 ObjectionDeck', type: 'free' as const, price: '免费', description: '按异议类型、渠道、语气与阶段生成四步成交回应。', href: 'https://chenxiaolong-maxgent.github.io/objectiondeck-cn/', icon: MessagesSquare },
  ...
  { name: '评改 ReviewDock', type: 'free' as const, price: '免费', description: '生成评审邀请、反馈规则、冲突决定、改稿清单与版本确认。', href: 'https://chenxiaolong-maxgent.github.io/reviewdock-cn/', icon: ListChecks },
  { name: '进度通 StatusPulse', type: 'free' as const, price: '免费', description: '生成客户周报、进度邮件、风险说明、客户待办与下一里程碑。', href: 'https://chenxiaolong-maxgent.github.io/statuspulse-cn/', icon: RadioTower },
  ...
  { name: 'ProspectKit PRO', type: 'product' as const, price: '¥29', description: '3 份 Word 拓客研究、四步冷启动话术与跟进转化复盘模板。', href: 'https://github.com/chenxiaolong-maxgent/prospectkit-cn/issues/new?template=buy-prospectkit-pro.yml', icon: Send },
  { name: 'AuditPage PRO', type: 'product' as const, price: '¥29', description: '3 份 Word 公开证据研究、一页诊断、发送跟进与小实验复盘模板。', href: 'https://github.com/chenxiaolong-maxgent/auditpage-cn/issues/new?template=buy-auditpage-pro.yml', icon: FileText },
  { name: 'PilotOffer PRO', type: 'product' as const, price: '¥29', description: '3 份 Word 付费试单设计、行业示例、交付验证与升级复盘模板。', href: 'https://github.com/chenxiaolong-maxgent/pilotoffer-cn/issues/new?template=buy-pilotoffer-pro.yml', icon: ClipboardCheck },
  { name: 'BriefPilot PRO', type: 'product' as const, price: '¥29', description: '3 份 Word 会前问卷、需求访谈、会议纪要、需求基线与方案交接模板。', href: 'https://github.com/chenxiaolong-maxgent/briefpilot-cn/issues/new?template=buy-briefpilot-pro.yml', icon: ScanSearch },
  { name: 'CapacityFlow PRO', type: 'product' as const, price: '¥29', description: '6 张 Excel 工作表，把项目、任务和 13 周滚动容量放进同一套系统。', href: 'https://github.com/chenxiaolong-maxgent/capacityflow-cn/issues/new?template=buy-capacityflow-pro.yml', icon: CalendarRange },
  { name: 'DealBoard PRO', type: 'product' as const, price: '¥29', description: '6 张 Excel 工作表，把商机、跟进和 12 个月成交预测放进同一套系统。', href: 'https://github.com/chenxiaolong-maxgent/dealboard-cn/issues/new?template=buy-dealboard-pro.yml', icon: Target },
  { name: 'ObjectionDeck PRO', type: 'product' as const, price: '¥29', description: '3 份 Word 高频异议回复、诊断跟进与话术迭代模板。', href: 'https://github.com/chenxiaolong-maxgent/objectiondeck-cn/issues/new?template=buy-objectiondeck-pro.yml', icon: MessagesSquare },
  ...
  { name: 'ReviewDock PRO', type: 'product' as const, price: '¥29', description: '3 份 Word 反馈收集、冲突决策、改稿台账、版本确认与下一轮审批模板。', href: 'https://github.com/chenxiaolong-maxgent/reviewdock-cn/issues/new?template=buy-reviewdock-pro.yml', icon: ListChecks },
  { name: 'StatusPulse PRO', type: 'product' as const, price: '¥29', description: '3 份 Word 客户周报、风险延迟、决策请求与月度复盘模板。', href: 'https://github.com/chenxiaolong-maxgent/statuspulse-cn/issues/new?template=buy-statuspulse-pro.yml', icon: RadioTower },
  ...
  { name: '接单经营全家桶', type: 'product' as const, price: '¥69', description: '42 份 Word、4 套 Excel 与完整项目经营工作流。', href: 'https://github.com/chenxiaolong-maxgent/freelance-ops-kit/issues/1', icon: PackageCheck, featured: true },
  { name: '定制拓客冲刺', type: 'service' as const, price: '¥199 起', description: '按服务、目标画像与真实证据定制 10 个角度、四步触达序列、记录表与一次修改。', href: 'https://github.com/chenxiaolong-maxgent/prospectkit-cn/issues/new?template=outreach-sprint-service.yml', icon: Send },
  { name: '定制一页诊断', type: 'service' as const, price: '¥199 起', description: '基于一个公开页面、流程或内容，交付证据明确的一页诊断与低风险下一步。', href: 'https://github.com/chenxiaolong-maxgent/auditpage-cn/issues/new?template=custom-audit-service.yml', icon: FileText },
  { name: '定制试单设计', type: 'service' as const, price: '¥199 起', description: '按真实机会压缩试单范围，明确交付、价格、反馈边界、验证信号与升级路径。', href: 'https://github.com/chenxiaolong-maxgent/pilotoffer-cn/issues/new?template=custom-pilot-service.yml', icon: ClipboardCheck },
  { name: '客户访谈方案定制', type: 'service' as const, price: '¥99 起', description: '按行业、客单价和流程定制问题树、风险追问、纪要结构与确认话术。', href: 'https://github.com/chenxiaolong-maxgent/briefpilot-cn/issues/new?template=discovery-service.yml', icon: ScanSearch },
  { name: '项目产能系统配置', type: 'service' as const, price: '¥99 起', description: '按项目类型配置容量口径、阶段、任务分类、预警阈值与启动示例。', href: 'https://github.com/chenxiaolong-maxgent/capacityflow-cn/issues/new?template=capacity-setup-service.yml', icon: CalendarRange },
  { name: '销售管道配置', type: 'service' as const, price: '¥99 起', description: '按业务类型配置销售阶段、成交概率、跟进规则、预测口径与启动示例。', href: 'https://github.com/chenxiaolong-maxgent/dealboard-cn/issues/new?template=pipeline-setup-service.yml', icon: Target },
  { name: '客户异议方案', type: 'service' as const, price: '¥99 起', description: '按业务与真实对话定制异议诊断、四步回应、跟进节奏与礼貌关闭方案。', href: 'https://github.com/chenxiaolong-maxgent/objectiondeck-cn/issues/new?template=objection-plan-service.yml', icon: MessagesSquare },
  ...
  { name: '客户反馈工作流定制', type: 'service' as const, price: '¥99 起', description: '按交付类型定制反馈入口、分类规则、冲突决定、改稿台账与版本命名。', href: 'https://github.com/chenxiaolong-maxgent/reviewdock-cn/issues/new?template=feedback-workflow-service.yml', icon: ListChecks },
  { name: '客户汇报工作流定制', type: 'service' as const, price: '¥99 起', description: '按项目类型定制周报字段、风险升级规则、决策入口和月度复盘节奏。', href: 'https://github.com/chenxiaolong-maxgent/statuspulse-cn/issues/new?template=client-reporting-service.yml', icon: RadioTower },
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section id="start" className="mx-auto max-w-[1440px] px-5 py-8 lg:px-8 lg:py-12">
        <div className="grid gap-8 lg:grid-cols-[0.93fr_1.07fr] lg:items-end">
          <div><p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-primary">Freelance operating stack</p><h1 className="max-w-3xl text-4xl font-semibold tracking-[-0.055em] sm:text-6xl">别再到处找模板。<br />先说你卡在哪里。</h1><p className="mt-5 max-w-xl text-base leading-7 text-muted-foreground">从冷启动拓客、公开证据诊断、付费试单、需求访谈、产能排期、销售预测、客户异议、报价、客户启动、反馈改稿、进度汇报到正式验收、回款与客户口碑，按当前问题进入最短解决路径。所有免费工具无需登录。</p><div className="mt-6 flex flex-wrap gap-4 text-xs font-semibold text-muted-foreground"><span><strong className="text-foreground">19</strong> 个免费工具</span><span><strong className="text-foreground">18</strong> 个成品商品</span><span><strong className="text-foreground">16</strong> 项产品化服务</span></div></div>
      ...
      <section className="border-y border-border bg-[#ebe9dc] px-5 py-16 lg:px-8 lg:py-20">
        <div className="mx-auto grid max-w-[1200px] items-center gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">One operating path</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em] sm:text-5xl">免费解决眼前问题，系统解决反复问题。</h2><p className="mt-5 text-base leading-7 text-muted-foreground">只遇到一次卡点，先用免费工具；如果同类问题反复发生，直接使用成品模板或完整经营系统。没有订阅，也不需要先买才能试。</p><div className="mt-7 grid gap-3 sm:grid-cols-2">{['先研究对象，再冷启动触达', '先给一页证据，再谈完整项目', '先卖可验证试单，再扩大合作', '先问需求，再做方案', '先看容量，再承诺开工', '先算预测，再补销售缺口', '先诊断异议，再谈优惠', '先算底线，再发报价', '先对齐启动，再开始交付', '先收口反馈，再开始改稿', '先发周报，再等客户追', '先写范围，再接受变更', '先写验收，再关闭项目', '先收尾款，再问口碑'].map((item) => <div key={item} className="flex items-center gap-2 text-sm font-medium"><Check className="size-4 text-primary" />{item}</div>)}</div></div>
        </div>
      </section>
